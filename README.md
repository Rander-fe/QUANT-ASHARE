...
Walk-Forward 模拟真实投资：每一步只用过去数据训练，隔开未来收益标签对应的 purge gap，再预测未来数据。生产路径为：

```text
过去750日训练 → purge → 60日局部验证/早停 → purge → 未来20日样本外预测
```

预测完成后窗口向前移动20个交易日，重新训练新模型，重复直到覆盖目标区间。项目另有年度5+1+1滚动分析和多折Purged调参，分别用于年度稳定性和超参数稳健性，不能与生产750/60/20窗口混称。详细解释见 [PROJECT_INTRO.md](PROJECT_INTRO.md) 第8.8节。
...
```text
LightGBM训练：MSE
Optuna调参：平均RankIC + 0.05 × 扣10bps成本后的Top10%组合效用
最终验收：净Sharpe、年化超额、最大回撤、换手率、成本和风险暴露
```

简单说，LightGBM用 MSE 学习，Optuna 用平均 RankIC 加经济效用选参数，最后用真实组合指标判断策略是否可用。
...
|---|---|---|---:|---:|---:|---:|---:|
| 5日30因子测试复算 | `label_ret_5` / 25+5=30 | 2025-01-02至2026-08-13 | 31.80% | 13.76% | 18.04% | 1.285 | -19.75% |

该结果来自30因子、`label_ret_5`、5日调仓测试复算，包含10bps交易成本。详细结果和审计文件见 [PROJECT_INTRO.md](PROJECT_INTRO.md) 第8.14.1.3节。
